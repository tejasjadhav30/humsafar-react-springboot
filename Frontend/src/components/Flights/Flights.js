...
const FlightForm = ({ type }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:8081/service1/flights");
      ...
      <section id="first-tab-group" className="tabgroup">
        <div id="tab1">
          <div className="submit-form">
            <h4>Check availability for Flight:</h4>
          ...
          <div className="result-list">
            {filterFlights().map((item, index) => (
              <div key={index} className="result-item">
                ...
                <Link to={`/booking/flights/${item.id}`}>
                  <button>Book</button>
                </Link>
              </div>
            ))}
